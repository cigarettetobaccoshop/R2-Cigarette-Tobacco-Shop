...
function buildProductCardHTML(p, idx) {
  ...
  return '<li class="product type-product status-publish instock bg-white dark:bg-white/5 rounded-3xl p-6 border border-slate-200 dark:border-white/10 card-premium card-glow relative overflow-hidden flex flex-col justify-between group card-enter' + (isResmi ? ' product-card-resmi' : '') + '" style="animation-delay:' + (idx * 40) + 'ms" data-pid="' + p.id + '">' +
    actions + ph +
    ...
    '<h3 class="woocommerce-loop-product__title text-lg font-serif font-bold text-deep dark:text-white leading-tight mb-1 group-hover:text-gold transition-colors">' + escapeHtml(p.name) + '</h3>' +
    (isResmi ? '<p class="text-[10px] text-slate-500 font-medium mb-2 italic">' + escapeHtml(p.segmentName) + '</p>' : '') +
    '<div class="flex items-end justify-between gap-2 flex-wrap"><span class="price text-2xl font-black text-deep dark:text-white font-mono tracking-tighter"><span class="woocommerce-Price-amount amount">' + formatRupiah(p.price) + '</span><span class="text-[10px] text-slate-400 font-sans font-medium ml-1">/slop</span></span><span class="stock in-stock">Stok Tersedia</span></div></div>' +
...
function renderCheckoutReviewTable() {
  ...
  body.innerHTML = cart.length ? cart.map(function (i) {
    return '<tr><td class="product-name">' + escapeHtml(i.name) + ' <strong class="product-quantity">&times; ' + i.qty + '</strong></td><td class="product-total"><span class="woocommerce-Price-amount amount">' + formatRupiah(i.price * i.qty) + '</span></td></tr>';
